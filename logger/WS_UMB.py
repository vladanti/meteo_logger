...
class WS_UMB:
	def __init__(self, device='', baudrate=19200, wait=10): #device='COM4'
		import serial
		delayed = False
	...
	def send_request(self, receiver_id, class_id, command, command_version, payload):
		...
		# Check class ID for WS and VA sensors series
		if class_id == 7:
			TO_CLASS = b'\x70'
		elif class_id == 8:
			TO_CLASS = b'\x80'
	...
	# RESET PRECIPITATION SECTION
	def send_reset(self, receiver_id):
		...
		payload = b'\x13' # custom reset payload
		...
		COMMAND = b'\x25' # reset command
		COMMAND_VERSION = b'\x10'
		...
		try:
			...
			print('Error. Reset command was not sent')
		else:
			print('Sucsess. Reset command sent to device')
	...
	def onlineDataQueryWS (self, receiver_id, channel):
		return self.send_request(receiver_id, 7, 35, 16, int(channel).to_bytes(2,'little'))

	def onlineDataQueryVA (self, receiver_id, channel):
		return self.send_request(receiver_id, 8, 35, 16, int(channel).to_bytes(2,'little'))

	def resetWSSensor (self, receiver_id):
		return self.send_reset(receiver_id)
